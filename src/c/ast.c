...
void generate_chart_png(const ChartSpec *c) {
    ...
    char cmd[4096];
    char filename[32];
    const char *python_exec = getenv("MDCHART_PYTHON");
    if (!python_exec || strlen(python_exec) == 0) {
        python_exec = "python3";
    }
    ...
    snprintf(cmd, sizeof(cmd), "%s src/c/generate_chart.py %s %s \"%s\" \"%s\"", 
             python_exec, c->type, filename, c->x_field, c->y_field);
